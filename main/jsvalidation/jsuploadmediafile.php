...
if( isset( $_SESSION['logged_in'] ) && $_SESSION['logged_in'] && $_SESSION['logged_in_user_data']['admin']=== 1 ) {
    $request_from_file = basename( $_SERVER['HTTP_REFERER'] );
    if( request_check_from_page( $request_from_file ) ) {
        if (isset($_FILES['fileInput']) && isset($_POST)) {
            ...
            if ($uploadOk == 0) {
                ...
                if (move_uploaded_file($_FILES["fileInput"]["tmp_name"], $targetFile)) {
                    $result = array(
                        'success' => true,
                        'image_name' => basename($_FILES["fileInput"]["name"]),
